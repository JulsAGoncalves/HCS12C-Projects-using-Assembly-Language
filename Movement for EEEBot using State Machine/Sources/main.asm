;*****************************************************************
;* STATE MACHINE FOR EEEBOT TO MOVE ABOUT                        *
;* HE'S ALIVE, HE'S ALIIIVVVVVVEEEEEE, THANKS TO ME!!            *
;* IF ALL GOES WELL, HE SHOULD BE STARTING PRE SCHOOL SOON       *
...
;*******************************************************************
;HERE
LCD_DAT       EQU   PORTB                     ; DEFINES DATA TO BE SENT TO LCD
LCD_CNTR      EQU   PTJ                       ; LCD SCREEN CONTROL PORT
LCD_E         EQU   $80                       ; FOR LCD ENABLE PIN
LCD_RS        EQU   $40                       ; LCD REGISTER SET SIGNAL PIN FOR WHICH MODE (DATA OR COMMAND)
;^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
NO_BLANK      ds.b  1
;HERE                         ; Used in ’leading zero’ blanking by BCD2ASC
BCD_SPARE     RMB   10                        ; EXTRA SPACE FOR PERHAPS A DECIMAL POINT OR NULL TERMINATION IN STRING
...
; subroutine section
;*******************************************************************
; WE MAKE AN IF ELSE LOOP CHECKING IF WE'RE IN THE RIGHT STATE, AND IF NOT, BRANCHING TO CHECK IF WE'RE IN THE NEXT
...
; DISPLAYING VOLTAGE READINDS ON LCD
